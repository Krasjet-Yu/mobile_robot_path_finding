...
int main(int argc, char** argv)
{
    ...
    nh.param("map/resolution",    _resolution,   0.2);  // 点云地图分辨率
    
    nh.param("map/x_size",        _x_size, 50.0);       // 点云地图x大小
    nh.param("map/y_size",        _y_size, 50.0);       // 点云地图y大小
    nh.param("map/z_size",        _z_size, 5.0 );       // 点云地图z大小
    
    nh.param("planning/start_x",  _start_pt(0),  0.0);  // 起始点
    nh.param("planning/start_y",  _start_pt(1),  0.0);  
    nh.param("planning/start_z",  _start_pt(2),  0.0);

    _map_lower << - _x_size/2.0, - _y_size/2.0,     0.0; // 点云地图的最低边界
    _map_upper << + _x_size/2.0, + _y_size/2.0, _z_size; // 点云地图的最大边界
    
    _inv_resolution = 1.0 / _resolution;
    
    _max_x_id = (int)(_x_size * _inv_resolution);        // 点云地图尺寸 / 分辨率 = 栅格地图栅格的尺寸
    _max_y_id = (int)(_y_size * _inv_resolution); 
    _max_z_id = (int)(_z_size * _inv_resolution);

    // 构造一个AstarPathFinder的类
